...
function UpCollege() {
    var data = {};
    var Info = {};
    data.tableName = "College";
    // Info.cid = Serch("t");
    Info.cname = Serch("cname");
    Info.cstatus = Serch("cstatus");
    Info.cremark = Serch("cremark");
...
function ShowTeachers() {
    ...
    var data = {"tableName": "College","name": "", "currentPage": 1};
...
function TeacherFunction() {
    $(function () {
        ...
        $("#Select").click(function () {
            var code = $("#code").val();
            var name = $("#name").val();
            var data = {"tableName": "College","name": name, "currentPage": 1};
            var table = getPage(data);
            if (table.code == 1) {
                TeachresTable(table.data.list);
                Refresh();
...
function TeacherModify() {
    ...
    var data = {"tableName": "College", "code": obj.cid, "name": "", "currentPage": 1};
    var table = getPage(data);
    var list = table.data.list[0];
    $("#cid").val(list.cid);
    $("#cname").val(list.cname);
    $("#cremark").val(list.cremark);
    $("#cstatus").val(list.cstatus);
    $("#cstatus option:contains(" + list.cstatus + ")").prop("selected", true);
    Refresh();
}
...
function ShowModify(id) {
    ...
    layui.use('layer', function () {
        ...
        layer.open({
            ...
            , content: '/SpringBoot-SIMS/static/html/UpdateCollegeInfo.jsp'
            , area: ['650px', '500px']
        });
    });
}
        ...
        laypage.render({
            ...
            , jump: function (obj, first) {
                ...
                if (!first) {
                    var code = $("#code").val();
                    var name = $("#name").val();
                    var data = {"tableName": "College","name": name, "currentPage": obj.curr};
                    var table = getPage(data);
                    if (table.code == 1) {
                        TeachresTable(table.data.list);
                        Refresh();
...
function TeachresTable(data) {
    if (data != null) {
        ...
        text += "</div></th>"
        text += "<th style='width: 240px'>学院名称</th><th>状态</th><th>备注</th><th style='width: 240px'>操作</th>";
        text += "</tr></thead>";
        text += "<tbody>";
        for (var i = 0; i < data.length; i++) {
            text += "<tr name=\'" + data[i].cid + "\'>";
            text += "<td><div class=\"layui-form\"> <input type=\"checkbox\" name=\"checkbox\" title=\"\" lay-skin=\"primary\" >";
            text += "</div></td>"
            text += "<td>" + data[i].cname + "</td>";
            text += "<td>" + (data[i].cstatus == '1' ? '启用':'停用')+ "</td>";
            text += "<td>" + data[i].cremark + "</td>";
            text += "<td  name=\'" + data[i].cid + "\'>";
